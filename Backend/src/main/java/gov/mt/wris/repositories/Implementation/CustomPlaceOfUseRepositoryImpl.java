...
import gov.mt.wris.repositories.CustomPlaceOfUseRepository;
...
@Repository
public class CustomPlaceOfUseRepositoryImpl implements CustomPlaceOfUseRepository {
    ...
    public BigDecimal getNextPlaceOfUseId(BigDecimal purposeId) {
        ...
        Integer ac = session.doReturningWork(
                connection -> {
                    try (CallableStatement callableStatement = connection.prepareCall("{ ? = call WRD_FORM_UTILITIES.wrd_next_puse_id_seq(?) }")) {
    ...
    public Integer reNumberPlaceOfUse(String sortCode, BigDecimal waterRightId, BigDecimal versionId) {
        ...
        Integer ac = session.doReturningWork(
                connection -> {
                    try (CallableStatement callableStatement = connection.prepareCall("{ ? = call WRD_COMMON_FUNCTIONS.renumber_place_of_use(?, ?, ?) }")) {
                        callableStatement.registerOutParameter(1, Types.INTEGER);
    ...
    public Integer replicatePodsPlus(BigDecimal purposeId) {
        ...
        Integer ac = session.doReturningWork(
                connection -> {
                    try (CallableStatement callableStatement = connection.prepareCall("{ ? = call WRD_COMMON_FUNCTIONS.replicate_pods_plus(?) }")) {
                        callableStatement.registerOutParameter(1, Types.INTEGER);
                        callableStatement.setBigDecimal(2, purposeId);
                        callableStatement.execute();
                        return callableStatement.getInt(1);
